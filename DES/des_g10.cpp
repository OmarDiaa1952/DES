...
int main(int argc, char *argv[]) {
  ...
  string x, K, condition , fileName ,str , output;
...
// starting of reading the file & encrypt line by line not store the all content of the file into string
    ...
    str=storeData(fileName);
    if(condition == "encrypt"){
    printf("Cipher Text is loaded into the file Encryption.txt : ..........");
    long long t1 = __rdtsc();
    clock_t start = clock();
    ofstream Efile(output , ios::binary );
    // string result ="";
      for (int i = 0; i < str.size();i+=8 )
      {
        u64  plainText = ASCII2u64(str.substr(i,8));
        u64 cipher = encrypt(plainText, roundKeys);
        // result += u64ToASCII(cipher);
        Efile<<u64ToASCII(cipher);
      }
    // ofstream Efile( output , ios::binary );
    //   Efile<<result;
